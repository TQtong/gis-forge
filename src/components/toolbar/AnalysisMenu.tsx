...
export interface AnalysisMenuProps {
    /** 选中某分析项时回调；未接 UI 层时默认 `console.info`。 */
    onSelectAnalysis?: (id: AnalysisDialogId) => void;
}

/**
 * Dropdown of spatial analysis tools。无 store 时通过可选回调或控制台占位。
 *
 * @returns Toolbar button + anchored menu.
 */
export function AnalysisMenu({ onSelectAnalysis }: AnalysisMenuProps = {}): ReactElement {
    const notify = onSelectAnalysis ?? ((id: AnalysisDialogId) => console.info('[AnalysisMenu] selected', id));
