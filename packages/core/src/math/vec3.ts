 ...
 * @param y - Y 分量，默认 0
 * @param z - Z 分量，默认 0
 * @returns 新分配的 Float32Array(3)
 *
 * @example
 * const v = vec3.create(1, 2, 3); // Float32Array [1, 2, 3]
 */
export function create(x = 0, y = 0, z = 0): Vec3f {
    // 分配 3 元素 Float32Array 并设初值
...
/**
 * 从任意类数组中提取 3 个连续元素构造 Vec3f。
 * 用于从交错顶点数据或平铺坐标数组中提取 3D 坐标。
 *
 ...
 * const v = vec3.fromArray(positions, 3); // Float32Array [1, 2, 3]
 */
 ...
 *
 * @example
 * const snapshot = vec3.create();
 * vec3.copy(snapshot, cameraPosition);
 */
 ...
 *
 * @example
 * vec3.set(v, 1, 0, 0); // v = [1, 0, 0]（X 轴单位向量）
 */
 ...
 *
 * @example
 * vec3.add(result, [1, 2, 3], [4, 5, 6]); // result = [5, 7, 9]
 */
 ...
 *
 * @example
 * vec3.sub(result, [5, 7, 9], [1, 2, 3]); // result = [4, 5, 6]
 */
 ...
 *
 * @example
 * vec3.scale(result, [1, 2, 3], 2); // result = [2, 4, 6]
 */
 ...
 * vec3.cross(result, [1, 0, 0], [0, 1, 0]); // result = [0, 0, 1]
 */
 ...
 * @example
 * const n = vec3.create();
 * vec3.normalize(n, [0, 0, 5]); // n = [0, 0, 1]
 */
 ...
 *
 * @example
 * vec3.negate(result, [1, -2, 3]); // result = [-1, 2, -3]
 */
 ...
 *
 * @example
 * vec3.lerp(result, [0, 0, 0], [10, 10, 10], 0.25); // result = [2.5, 2.5, 2.5]
 */
 ...
 *
 * @example
 * vec3.squaredDistance([0, 0, 0], [1, 2, 2]); // 9
 */
 ...
 *
 * @example
 * vec3.multiply(result, [2, 3, 4], [5, 6, 7]); // result = [10, 18, 28]
 */
 ...
 *
 * @example
 * vec3.min(result, [1, 5, 3], [3, 2, 7]); // result = [1, 2, 3]
 */
 ...
 *
 * @example
 * vec3.max(result, [1, 5, 3], [3, 2, 7]); // result = [3, 5, 7]
 */
 ...
 * vec3.transformMat4(projected, worldPos, vpMatrix);
 */
 ...
 * // 绕 Y 轴旋转 90° 的四元数
 * const q = new Float32Array([0, 0.7071, 0, 0.7071]);
 * vec3.transformQuat(rotated, [1, 0, 0], q); // rotated ≈ [0, 0, -1]
 */
 ...
 *
 * @example
 * vec3.equals([1, 2, 3], [1, 2, 3.0000001]); // true
 */
