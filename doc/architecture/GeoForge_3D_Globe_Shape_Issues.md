...
> **GeoForge 对接**：Globe Pipeline v2 `§一 WGS84 常量对象`——所有坐标转换函数以 `Ellipsoid` 参数化。未来支持非 WGS84 只需替换常量对象。
...
> **GeoForge 对接**：与 GeoForge 当前 bug 完全一致——`viewRTE[14] = -26369352`。Globe Pipeline v2 `§二 computeCamera3D` 的 `mat4_lookAt([0,0,0], targetRTE, upECEF)` 已修复。
...
> **GeoForge 对接**：Globe Pipeline v2 `§六 morph 过渡` 中两套坐标系（flatPosRel + globePosRel）归一化到相对空间解决了这个问题。
...
替代方案（GeoForge 当前选择）：裙边（skirt）遮挡接缝——更简单，
...
> **GeoForge 对接**：Globe Pipeline v2 `§三 tessellateGlobeTile` 基于弧度角细分 + `isNorthPole/isSouthPole` 扇形三角形处理。
...
> **GeoForge 对接**：Globe Pipeline v2 `§四.2 isTileVisible_Horizon` 用 `dot(camToTile, normal) < margin` 实现，margin 基于瓦片角半径。与 CesiumJS occludee point 的简化等价。
...
跨日期变更线的瓦片（如 lngMin=170°, lngMax=-170°）：
  ...
  解法 2（GeoForge）：在 coveringTilesGlobe 中，
...
> **GeoForge 对接**：Globe Pipeline v2 `§四.4 coveringTilesGlobe` 步骤 2 中 `if (mxLng - mnLng > 180) { mnLng = -180; mxLng = 180; }`。极地退化三角形在 `§三.2 tessellateGlobeTile` 的 `isNorthPole/isSouthPole` 分支处理。
...
> **GeoForge 对接**：Globe Pipeline v2 `§六 computeMorphVertices` 将两套坐标归一化到以 center 为原点的相对空间（flatPosRel + globePosRel），数值量级一致后 mix 产生平滑过渡。与 **Tile Solutions §方案六**（cross-fade）联动——morph 期间旧模式瓦片保留直到新模式瓦片就绪。

---

## 八、对 GeoForge 的设计要求总结
...
| P1 | 2 | Globe picking（GPU depth readback） | CesiumJS #8481 | 新增 `@geoforge/globe-picking` |
...
### 与其他 GeoForge 文档的交叉引用

| 本文 Issue | 相关 GeoForge 文档 | 交叉点 |
|-----------|-------------------|--------|
