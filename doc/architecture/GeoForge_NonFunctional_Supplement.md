# GIS-Forge 架构补充 — 可扩展性 / 可维护性 / 性能优化 / 易用性
...
const ALLOWED_DEPS: Record<string, string[]> = {
  '@gis-forge/core':       [],                              // L0 不依赖任何包
  '@gis-forge/gpu':        ['@gis-forge/core'],              // L1+L2 只依赖 L0
  '@gis-forge/runtime':    ['@gis-forge/core', '@gis-forge/gpu'],  // L3
  '@gis-forge/scene':      ['@gis-forge/core', '@gis-forge/gpu', '@gis-forge/runtime'],
  '@gis-forge/extensions': ['@gis-forge/core', '@gis-forge/gpu', '@gis-forge/runtime', '@gis-forge/scene'],
  // preset-* 可以依赖全部
};
...
从 MapLibre GL / CesiumJS / Leaflet 迁移到 GIS-Forge 的路径未定义。
...
const DEVELOPER_HINTS: Record<string, DeveloperHint> = {
  'GPU_SHADER_COMPILE': {
    ...
    docUrl: 'https://gis-forge.dev/docs/troubleshooting#shader-errors',
  },
  'CONFIG_UNKNOWN_LAYER_TYPE': {
    ...
    docUrl: 'https://gis-forge.dev/docs/layers',
  },
  'GPU_BUFFER_OOM': {
    ...
    docUrl: 'https://gis-forge.dev/docs/performance#memory',
  },
};
...
//    📖 文档：https://gis-forge.dev/docs/troubleshooting#shader-errors
...
Level 3 — 3D 模式（改 1 行）
  import { Map } from '@gis-forge/preset-full';
...
```typescript
// 新增可选包：@gis-forge/compat-maplibre
// 让 MapLibre GL JS 的代码以最小改动在 GIS-Forge 上运行

export function createMapLibreCompat(map: Map2D): MapLibreCompatMap {
  // 代理 MapLibre 的 API 调用到 GIS-Forge 对应方法
  // 处理差异：
  //   - MapLibre: queryRenderedFeatures 同步 → GIS-Forge: 异步（包装为 sync stub + warning）
  //   - MapLibre: map.getCanvas().getContext('webgl') → GIS-Forge: 不支持（抛出友好错误）
